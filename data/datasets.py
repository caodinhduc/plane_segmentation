...
if __name__ == "__main__":
    ...
    for idx, data_ele in enumerate(data_loader):
        ...
        intrinsic_matrix = torch.stack([gt_instances[img_idx]['k_matrix'] for img_idx in range(len(gt_instances))], dim=0).cuda(2)
        ...
        for i in range(len(imgs)):
            inst = gt_instances[i]
            masks = inst['masks']
            offsets = inst['plane_paras'][:,3:].cuda(2).double()
            normals = inst['plane_paras'][:,:3].cuda(2).double()
            total = 0
